...
function MoreInfoData({ bookInfo }: Iprops) {
  const {
    author,
    country,
    imageLink,
  ...
  return (
    <div className=" w-full h-full ">
      <div className="p-2 flex flex-col items-start justify-between bg-white border border-gray-200 rounded-lg shadow md:flex-row  hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 gap-3 h-full ">
        {imageLink && typeof imageLink === "string" && (
          <img
            className="object-cover w-full rounded-t-lg h-96 md:h-full md:w-[60%] sm:h-full md:rounded-none md:rounded-s-lg"
            src={imageLink}
            alt={title}
          />
        )}
